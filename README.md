...
The script rebuilds 17 article pages, copies or reuses the original Markdown in `blog/source/`, and copies the editable diagrams into `blog/assets/diagrams/`. The bundled tactile-reading notes are kept in `blog/source/` so they remain available when the external BrainCo documentation directory is not present.
